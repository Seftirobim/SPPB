...
class Barang2 extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('kode_brg,kd_brg, nm_barang, satuan, qty_request, qty_realisasi, keterangan', 'required'),
			array('qty_request, qty_realisasi', 'numerical', 'integerOnly'=>true),
			array('kode_brg, nm_barang, satuan, keterangan', 'length', 'max'=>30),
			array('id, updateType', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, kode_brg,kd_brg, nm_barang, satuan, qty_request, qty_realisasi, keterangan', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('kd_brg',$this->kd_brg);
		$criteria->compare('kode_brg',$this->kode_brg,true);

...
class Barang2 extends CActiveRecord
{
	 ...
	 */
	public function ada($sql){
	// $sql =  "select * from barang where kode_barang = 'ATK002'";
	// $barang = Yii::app()->db->createCommand($sql)->queryAll();
	// 	if($this->kd_brg != $barang){
	// 		$this->addError('kd_brg','tidak boleh');
	// 	}
	// 	return false;
	}
	...
	public function rules()
	{
		...
		return array(
			...
			array('kd_brg, satuan', 'length', 'max'=>30),
			...
			array('id2','safe'),
			array('satuan','safe'),
			//array('kd_brg','match','pattern'=>'[^AT|^B]','message'=>'Kode Tidak Ada'),
			//array('kd_brg','match','pattern'=>'[^AT|^B]','message'=>'Kode Tidak Ada'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, kd_brg, id2, nm_barang, satuan, qty_request, qty_realisasi, keterangan', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'Sppbs2' => array(self::BELONGS_TO, 'Sppb2', 'id2'),
			'Biring2'=>array(
				self::HAS_MANY,
				'Barang',
				'kode_barang')
